...
def send_subscription_mail(email, request):
    ...
    * ``newsletter_subscription/subscription_email.html``: The body of the
    ...
    url = request.build_absolute_uri(
        reverse('webinar_subscription_subscribe', kwargs={
            'code': get_signer().sign(email),
        }))

    render_to_mail(
        'webinar_subscription/subscription_email',
        {
            'subscribe_url': url,
        },
        to=[email],
...
def send_unsubscription_mail(email, request):
    url = request.build_absolute_uri(
        reverse(
            'webinar_subscription_resubscribe',
            kwargs={
                'code': get_signer().sign(email),
            }))

    render_to_mail(
        'webinar_subscription/unsubscription_email',
        {
            'resubscribe_url': url,
        },
        to=[email],
