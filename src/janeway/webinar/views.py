...
conferences = {
       '20210501' : {
           'image_url': 'common/webinar-2021.png',
           'conference_date': 'May 1st 2021',
           'conference_time': '11h30-12h00pm (EDT)',
           'title': 'Towards a paradigm shift in applied human intelligence'
        }
       }       
    ...
    def clean(self):
        ...
        if self.backend.is_subscribed(email):
            raise forms.ValidationError(
                _('This address is already subscribed to this conference!'))
...
def form(request, slug, backend):
    # first check that the date match a scheduled conferences
    # /webinar/
    if not slug in conferences:
        return HttpResponse("Invalid conference ID.")
    ...
    return render(request, 'webinar_subscription/form.html', {
        'form': form,
...
def subscribe(request, code, backend):
    try:
        ...
        messages.error(request, _('We are sorry. This link is expired.'))
        return redirect('webinar_subscription_form')
    ...
    return render(request, 'webinar_subscription/subscribe.html', {
        'email': email,
        'form': form,
    })
...
def resubscribe(request, code, backend):
    ...
    return redirect('webinar_subscription_subscribe', code=code)
