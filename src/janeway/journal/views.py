...
@has_journal
@article_exists
def sponsor_article(request, identifier_type, identifier):
    #XXX fixme - jack
    article_object = submission_models.Article.get_article(request.journal, identifier_type, identifier)
    context = {'sponsor_form' : forms.SponsorArticleForm(article_object)}
    template = "journal/sponsor_article.html"
...
def become_sponsor(request):
    # register then set role as "sponsor"
    code = 'not-logged-in'
    message = _('You must login before you can become a reviewer. Click the button below to login.')
    if request.user and request.user.is_authenticated() and not request.user.is_sponsor(request):
        # We have a user, they are logged in and not yet a reviewer
        code = 'not-sponsor'
        message = _('You are not yet a sponsor for this journal. Click the button below to become a sponsor.')
    elif request.user and request.user.is_authenticated() and request.user.is_sponsor(request):
        # The user is logged in, and is already a reviewer
        code = 'already-sponsor'
        message = _('You are already a sponsor!')

    if request.POST.get('action', None) == 'go':
        request.user.add_account_role('sponsor', request.journal)
        messages.add_message(request, messages.SUCCESS, _('You are now a sponsor.'))
        return redirect(reverse('core_dashboard'))

    template = 'journal/become_sponsor.html'
    context = {
        'code': code,
        'message': message,
    }
...
def become_reviewer(request):
    """
    If a user is signed in and not a reviewer, lets them become one, otherwise asks them to login/tells them they
