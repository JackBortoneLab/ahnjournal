...
def send_reviewer_withdrawal_notice(**kwargs):
    ...
    if not skip:
        log_dict = {'level': 'Info', 'action_text': description, 'types': 'Review Withdrawal',
                    'target': review_assignment.article}
        notify_helpers.send_email_with_body_from_user(request, 'subject_review_withdrawal', review_assignment.reviewer.email,
...
def preprint_comment(**kwargs):
    ...
    if 'press_base_url' in request:
        url = request.press_base_url
    else:
        #Journal
        url = request.journal.full_url()
    email_text = 'A comment has been made on your article {article}, you can moderate comments ' \
                 '<a href="{base_url}{url}">on the journal site</a>.'.format(
                     article=article.title, base_url=url, url=reverse('preprints_comments',
                                                                                         kwargs={'article_id': article.pk}))
