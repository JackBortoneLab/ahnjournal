...
def start(request, type='preprint', author_mode=True):
    """
...
def submit_funding(request, article_id):
    ...
    if request.method == 'POST':
        ...
        if 'amount' in new_data:
            ...
            article.funding_total += amount
            article.save()
            request.session['amount'] = amount
        #context = {'amount': request.session['amount']}
        #return render(request, 'admin/submission/submit_funding_done.html',
    ...
    template = 'admin/submission/submit_funding_paypal.html'
    context = {
        'article': article,
        'form': form,
        'additional_fields': additional_fields,
...
#@login_required
def submit_funding_done(request, article_id):
    # just show the amount donated + funding_total
    article = get_object_or_404(models.Article, pk=article_id)
    template = 'admin/submission/submit_funding_done.html'
    context = {
        'article': article,
...
def edit_metadata(request, article_id):
    ...
    if request.POST:
        if 'add_funder' in request.POST:
            funder = core_models.Account.objects.create(
                username=request.POST.get('funder_name', default=''),
                #fundref_id=request.POST.get('funder_doi', default=''),
                #funding_id=request.POST.get('grant_number', default='')
            )
            funder.enable_sponsor = True
            funder.save()
            article.funders.add(funder)
            article.save()
