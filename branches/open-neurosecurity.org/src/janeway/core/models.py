...
class Account(AbstractBaseUser, PermissionsMixin):
    ...
    department = models.CharField(max_length=300, null=True, blank=True, verbose_name=_('Laboratory'))
    ...
    enable_public_profile = models.BooleanField(default=True, help_text='If enabled, your basic profile will be '
                                                'available to the public.')
    
    # Enable sponsor-mode so funder can donate funds to submitted articles
    enable_sponsor = models.BooleanField(default=False, help_text="If enabled, activate sponsoring mode.")
    ...
    def is_sponsor(self, request, journal=None):
        return self.check_role(request.journal, 'sponsor')
    ...
    def preprints(self):
        return submission_models.Article.preprints.filter(
                owner=self,
                stage=submission_models.STAGE_PREPRINT_PUBLISHED
                )

    def published_articles(self):
        articles = submission_models.Article.filter(
            owner=self,
            stage=submission_models.STAGE_PUBLISHED,
            #date_updated__lte=timezone.now(),
        )
...
class File(models.Model):
    ...
    def url(self):
        ...
        return reverse('serve_press_file', kwargs=url_kwargs)
