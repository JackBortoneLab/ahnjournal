...
def register(request):
    """
    Displays a form for users to register with the journal. If the user is registering on a journal we give them the Author role.
    :param request: HttpRequest object
    :return: HttpResponse object
    """
    ...
    if request.POST:
        ...
        if group == 'journal' and attr_form.is_valid():
            ...
            if group == 'journal' and journal.default_large_image:
                #FIXME
                path = journal.default_large_image.path
